function [obj_value,obj_grad]=ProgressG(S0,S1,  Z, G,U,V,E, rho, gamma1,gamma2,gamma3)
    [row,col] = size(Z);
    Ic = ones(col,1);
    Ir = ones(row,1);
    ...
    obj_sec = norm(G-V*G-E+(1/rho)*gamma1,'fro')^2;
    ...
    grad_fir = 2*(S0.*(G-Z));
    I = eye(row);
    grad_sec = rho * ((I-V)'*(G-V*G-E+(1/rho)*gamma1));
